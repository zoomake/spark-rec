...
object alsBatchRecommender {
  ...
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("alsBatchRecommender").setMaster("local")
...
//        sys.exit()
//    }
    val dateStr = "20160424 10"
//    val dateStr = args(0)

    val iterations = if (args.length > 1) args(1).toInt else 5

    val dataDir = "C:\\Users\\chinaso\\Desktop\\netflix"
//    val dataDir = "hdfs://master:9001/leechanx/netflix/"

    //cache()算子，将RDD从磁盘缓存到内存,cache只有一个默认的缓存级别MEMORY_ONLY，即将数据持久化到内存中，而persist可以通过传递一个 StorageLevel 对象来设置缓存的存储级别。
    val trainData = sc.textFile(dataDir + "trainingData.txt").map{ line =>
      val lineAttrs = line.trim.split(",")
      //userId,proudctId,rating
